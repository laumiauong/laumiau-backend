...
public class RelatorioAdmin extends JFrame {
    ...
    public RelatorioAdmin() {
        ...
        JLabel lblT = new JLabel("<html><div style='margin-bottom: 5px;'><b style='font-size: 22px;'>Dashboard Administrativo</b></div><font color='#6b7280' size='4'>Visão geral das atividades da ONG</font></html>");
        tPanel.add(lblT);
        gbc.gridx = 0; gbc.gridy = 0; gbc.gridwidth = 2; gbc.insets = new Insets(0, 0, 25, 0);
        mainContent.add(tPanel, gbc);

        // Cards — agora todos com JLabel dinâmico
        ...
        // Painel inferior
        gbc.gridy = 2; gbc.gridwidth = 1; gbc.weightx = 0.65; gbc.insets = new Insets(0, 0, 0, 20);
        ...
        // Timer: atualiza a cada 10 segundos automaticamente
...
private void carregarSolicitacoes() {
    ...
    try {
        ...
        carregarSolicitacoes(); // <- linha nova
    } catch (Exception e) {
    ...
    private JPanel criarNavbar() {
        ...
        menu.add(new JLabel("Dashboard") {{ setForeground(LARANJA_BASE); setFont(new Font("SansSerif", Font.BOLD, 14)); }});
        menu.add(new JLabel("Animais")   {{ setFont(new Font("SansSerif", Font.BOLD, 14)); }});
        menu.add(new JLabel("Adotantes") {{ setFont(new Font("SansSerif", Font.BOLD, 14)); }});
    ...
    private JPanel criarAcoesRapidas() {
        ...
        botoes.add(new OrangeButton("➕  Cadastrar Novo Pet"));
        botoes.add(new OrangeButton("📊  Gerenciar Animais"));
